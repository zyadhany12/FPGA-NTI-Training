module controller 
#(  parameter integer HLT=0, SKZ=1, ADD=2, AND=3, XOR=4, LDA=5, STO=6, JMP=7 )
(
...
always @(*) 
begin
  sel = 0; rd = 0; ld_ir = 0; inc_pc = 0; halt = 0;
  ld_pc = 0; data_e = 0; ld_ac = 0; wr = 0;
  case (phase)
    3'd0: sel = 1;

    3'd1: begin rd = 1; sel = 1; end

    3'd2: begin sel = 1; rd = 1; ld_ir = 1; end

    3'd3: begin sel = 1; rd = 1; ld_ir = 1; end

    3'd4: begin inc_pc = 1; halt = (opcode == HLT) ? 1 : 1'b0; end

    3'd5: begin if(opcode == ADD || opcode == AND || opcode == XOR || opcode == LDA) rd = 1; end

    3'd6: 
    begin
          if(opcode == ADD || opcode == AND || opcode == XOR || opcode == LDA) rd = 1; 
          else if(opcode == SKZ && zero == 1 ) inc_pc = 1; 
          else if(opcode == JMP) ld_pc = 1;
          else if(opcode == STO) data_e = 1;
    end

    3'd7:
    begin
        if(opcode == ADD || opcode == AND || opcode == XOR || opcode == LDA) begin rd = 1; ld_ac = 1; end 
        else if(opcode == JMP) ld_pc = 1;
        else if(opcode == STO) begin wr = 1; data_e = 1; end
    end
  endcase
